      ...
      >
            ...
            {profile.notifications.filter(n=>!n.is_read).slice(0,8).map(n=>(
              <View key={n.id} style={[s.notif, {flexDirection:'row', gap:10, alignItems:'flex-start'}]}>
                ...
                <View style={{flex:1}}>
                  ...
                  {n.created_at && <Text style={{fontSize:10,color:COLORS.text3,marginTop:2}}>{timeAgo(n.created_at)}</Text>}
                </View>
              </View>
            ))}
          </Section>
        ...
        <Section title="ℹ️ INFORMACIÓN LEGAL">
          {[
            ...
            ['Términos de Uso', () => openURL('https://web-production-a8023.up.railway.app/terms')],
            ['Código fuente (contribuir)', () => openURL('https://github.com/Kamsito1/precimap-app')],
          ].map(([label, onPress]) => (
